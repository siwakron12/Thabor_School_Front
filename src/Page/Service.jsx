...
export default function Service() {
    ...
    const fetchItem = async () => {
        try {
            const response = await fetch(`${apiUrl}workstructure/servepdf/${Id}/`);
            if (!response.ok) {
    ...
    const fetchImg = async () => {
        try {
            const response = await fetch(`${apiUrl}workstructure/uploads/`);
            if (!response.ok) {
    ...
    return (
        <>
            ...
            <Helmet>
                <meta charSet="utf-8" />
                <title>โครงสร้างงานบริหาร</title>
             
                </Helmet>
                <Nav />
                <div className="mx-auto mt-8 w-full lg:w-3/4 p-2">
                    <h1 className="text-red-600 text-3xl my-4">โครงสร้างงานบริหาร</h1>

                    {/* Display PDF */}
                    {man && (
                        <div className='w-full p-2 border-gray-600 border-2  '>
                            <iframe className="mt-4 w-full h-[600px] lg:h-[800px] " src={man}></iframe>
                            <a href={man} className="text-white bg-red-600 p-2 rounded-lg " target="_blank">ดูเต็มหน้า</a>
                        </div>
                    )}
