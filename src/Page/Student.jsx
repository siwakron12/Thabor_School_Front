...
export default function Student() {
    ...
    const fetchItem = async () => {
        try {
            const response = await fetch(`${apiUrl}student-detail/servepdf/${Id}/`);
            if (!response.ok) {
    ...
    const fetchImg = async () => {
        try {
            ...
            setImg(data);
            console.log(data[0].id) // Set `img` state directly with the array of images
            setId(data[0].id)
        ...
        <>
            <div className="font-thai">
                ...
                <div className="mx-auto mt-8 w-full lg:w-3/4 p-2">
                    <h1 className="text-red-600 text-3xl my-4">ข้อมูลนักเรียน</h1>
