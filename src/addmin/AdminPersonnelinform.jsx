...
export default function AdminPersonnelinform() {
  ...
  const fetchAlbums = async () => {
    try {
      const response = await fetch(`${apiUrl}personnelinform/uploads/`, {
        headers: {
  ...
  const onFileUpload = async (ev) => {
    ...
    try {
      const url = editMode ? `${apiUrl}personnelinform/uploads/${editId}/` : `${apiUrl}personnelinform/uploads/`;
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`${apiUrl}personnelinform/uploads/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <div className="flex w-full h-full p-2">
      ...
      <main className="ml-14">
        <h1 className='text-3xl'>ข้อมูลคุณครูเเละบุคลากรทางการศึกษา</h1>
