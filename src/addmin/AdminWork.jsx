...
export default function AdminWork() {
  ...
  const onFileUpload = async (ev) => {
    ...
    try {
      const url = editMode ? `${apiUrl}workstructure/uploads/${editId}/` : `${apiUrl}workstructure/uploads/`;
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`${apiUrl}workstructure/uploads/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <div className="flex w-full h-full p-2">
      ...
      <main className="ml-14">
        <h1 className='text-3xl'>แก้ไข้ข้อมูลโครงสร้างงานบริหาร</h1>
