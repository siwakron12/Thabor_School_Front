...
export default function AdminStudentcouncil() {
  ...
  const onFileUpload = async (ev) => {
    ...
    try {
      const url = editMode ? `${apiUrl}studentcouncil/uploads/${editId}/` : `${apiUrl}studentcouncil/uploads/`;
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`${apiUrl}studentcouncil/uploads/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <div className="flex w-full h-full p-2">
      ...
      <main className="ml-14">
        <h1 className='text-3xl'>แก้ไข้คณะกรรมการสภานักเรียน</h1>
