...
export default function AdminEducationboard() {
  ...
  const onFileUpload = async (ev) => {
    ...
    try {
      const url = editMode ? `${apiUrl}educationboard/uploads/${editId}/` : `${apiUrl}educationboard/uploads/`;
  ...
  const deleteItem = async (id) => {
    try {
      const response = await fetch(`${apiUrl}educationboard/uploads/${id}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  return (
    <div className="flex w-full h-full p-2">
      ...
      <main className="ml-14">
        <h1 className='text-3xl'>แก้ไข้คณะกรรมการศึกษาขึ้นพื้นฐาน</h1>
